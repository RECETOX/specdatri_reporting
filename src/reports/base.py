...
class ReportGenerator(ABC):
    ...
    def load_existing_report(self) -> Dict[str, Dict[str, int]]:
        ...
        with open(self.output_path, 'r', newline='', encoding='utf-8') as f:
            ...
            return {row[period_col]: {k: int(v) for k, v in row.items() if k != period_col and v} 
                    for row in reader}
    ...
    def filter_periods(self, all_periods: set, year: Optional[int] = None) -> list:
        ...
        return sorted(p for p in all_periods 
    ...
    def create_report(self, year: Optional[int] = None) -> None:
        ...
        entity_data = {entity: self.aggregate_data(path) for entity, path in files.items()}
        ...
        with open(self.output_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for period in periods:
                writer.writerow([period] + [data_matrix[period].get(e) or '' for e in entities])
        ...
        if existing_data:
            ...
            total_filled = sum(1 for p in periods for e in entities if data_matrix[p].get(e) is not None)
