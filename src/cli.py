...
@cli.command()
@click.option('--repository', default=None, help='Repository path in format OWNER/REPO (default: RECETOX/<PROJECT>)')
@click.option('--project', required=True, help='Project name')
@click.option('--repository-list', type=click.Path(), default='repository_list.tsv', help='Path to repository_list.tsv')
...
def add_repo(repository, project, repository_list, pypi, bioconda, cran, github):
    ...
    if not any([pypi, bioconda, cran, github]):
        click.echo('Error: At least one source flag must be specified (--pypi, --bioconda, --cran, or --github)')
        raise click.Exit(1)
...
@cli.command()
@click.option('--repository-list', type=click.Path(exists=True), default='repository_list.tsv',
...
@click.option('--tmp-dir', type=click.Path(exists=True), default='tmp',
